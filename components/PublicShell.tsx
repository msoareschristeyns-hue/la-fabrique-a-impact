   ...
   <header className="publicNav">
     <div className="publicNavInner">
       ...
       <nav className="publicDesktopNav" aria-label="Navigation principale">
         {nav.map(item=><Link key={item.href} href={item.href} onClick={()=>handleNavClick(item.href)} className={isActive(item.href)?'active':''}>{item.label}</Link>)}
       </nav>
     ...
     {open&&<nav className="publicMobileNav" aria-label="Navigation mobile">
       {nav.map(item=><Link key={item.href} href={item.href} onClick={()=>handleNavClick(item.href)} className={isActive(item.href)?'active':''}><b>{item.label}</b><ArrowRight/></Link>)}
     </nav>}
   </header>
