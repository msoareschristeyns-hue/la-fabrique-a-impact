...
async function persistMetadata(metadata: Metadata) {
  ...
  const { error } = await supabase
    .from('template_workspaces')
    .update({ values: { ...values, ...metadataPatch(metadata) }, updated_at: new Date().toISOString() })
...
function renderStrip(root: HTMLElement, metadata: Metadata) {
  ...
  strip.innerHTML = `
    ...
    <div><small>Validation</small><b>${metadata.validationDate || 'À définir'}</b></div>`;
...
async function createPanel(root: HTMLElement) {
  ...
  const context = await getWorkspace();
  let current = context?.workspace
    ? metadataFromValues((context.workspace.values || {}) as Record<string, unknown>)
    : { ...emptyMetadata };
  ...
  panel.innerHTML = `
    ...
    <label>Auteur<input data-meta="author" value="${current.author.replaceAll('"','&quot;')}" placeholder="Nom / fonction"></label>
    <label>Référent<input data-meta="referent" value="${current.referent.replaceAll('"','&quot;')}" placeholder="Nom / fonction"></label>
    <div class="docMetadataGrid">
      <label>Version<input data-meta="version" value="${current.version.replaceAll('"','&quot;')}" placeholder="V1"></label>
      <label>Statut<select data-meta="status"><option>Brouillon</option><option>À valider</option><option>Validé</option><option>Validé sous réserve</option><option>Archivé</option></select></label>
    </div>
    <label>Date de validation<input data-meta="validationDate" type="date" value="${current.validationDate.match(/^\d{4}-\d{2}-\d{2}$/)?.[0] || ''}"></label>
    <button type="button" class="docMetadataSave">Enregistrer les informations</button>
    <small class="docMetadataStatus"></small>`;

  const statusSelect = panel.querySelector<HTMLSelectElement>('[data-meta="status"]');
  if (statusSelect) statusSelect.value = current.status;

  const readForm = () => {
    const read = (name: keyof Metadata) => {
      const input = panel.querySelector<HTMLInputElement | HTMLSelectElement>(`[data-meta="${name}"]`);
      return input?.value.trim() || '';
    };
    current = {
      author: read('author'),
      referent: read('referent'),
      version: read('version') || 'V1',
      status: read('status') || 'Brouillon',
      validationDate: read('validationDate'),
    };
    renderStrip(root, current);
  };

  panel.querySelectorAll('input,select').forEach(field => field.addEventListener('input', readForm));
  ...
  panel.querySelector('.docMetadataSave')?.addEventListener('click', async () => {
    ...
    if (status) status.textContent = saved
      ? '✓ Informations enregistrées pour l’entreprise'
      : 'Le document doit être enregistré une première fois avant la synchronisation';
  });

  document.body.appendChild(panel);

  const repairAfterWorkspaceSave = new MutationObserver(async mutations => {
    const savedNow = mutations.some(mutation =>
      Array.from(mutation.addedNodes).some(node => node instanceof HTMLElement && (node.matches?.('.saveSuccess') || node.querySelector?.('.saveSuccess')))
    );
    if (savedNow) {
      const saved = await persistMetadata(current);
...
export default function DocumentMetadataPanel() {
  useEffect(() => {
    ...
    const refresh = () => {
      ...
      cleanupPanel?.();
      document.querySelector('.docMetadataPanel')?.remove();
      currentRoot?.querySelector('.docGovernanceStrip')?.remove();
      currentRoot = root;
      if (root) {
        const token = ++sequence;
        createPanel(root).then(cleanup => {
          if (token !== sequence) cleanup?.();
          else cleanupPanel = cleanup;
        });
      }
    };
